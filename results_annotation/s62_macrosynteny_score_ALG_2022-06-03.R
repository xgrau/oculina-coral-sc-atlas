...
# sps_list = c("Ocupat","Porlut","Amil","Acrpal","Acrcer","Pocver","Haloct")
...
for (n in 1:length(list_comparisons)) {
	...
	for (i in 1:length(sps_list)) {
		...
		gen_d = gen_d [ gen_d$seqnames %in% names(which(table(gen_d$seqnames) > 200)),  ]
	...
	for (i in 1:length(sps_list)) {
		...
		pdf(sprintf("%s/alg.%s.chrom-%s.pdf", out_fn, cmpnm, spi), height = 8, width = 6)
		...
		for (chr in colnames(ovs_i)) {
			...
			# which algs to plot
			keep_alg_ixs = which(sapply(1:nrow(jac_i_i), function(r) max(jac_i_i[r,]) >= 0.01))
			...
			if (do_plot) {
				...
				# barplot aggregated
				ovb_i_i_m_f = t(t(ovb_i_i_m) / apply(t(ovb_i_i_m), 1, sum))
				ovb_i_i_m_f [ is.na(ovb_i_i_m) ] = 0
				par(lwd = 0.5)
				barplot(ovb_i_i_m_f, col = colors_row [ rownames(ovb_i_i_m) ], las = 2, cex.names = 0.5, cex.main = 0.7, cex.axis = 0.7, ylab = "Fraction", cex.lab = 0.7, main = sprintf("ALG contribution to %s", chr), xlim = c(1,100), border = NA, space = 0)
				par(lwd = 1)
				...
				if (!is.null(dim(ovb_i_i_m_nvp))) {
					...
					ddd = t(data.frame(ovb_i_i_m_nvp))
					colnames(ddd) = row.names(ovb_i_i_m)
				} else {
					ovb_chi = list()
					ovb_chi$method = "Chisq fails"
					ovb_chi$p.value = 1
					n_bins = 1
					ddd = data.frame(ovb_i_i_m_nvp)
					colnames(ddd) = row.names(ovb_i_i_m)
				}
				# save
				write.table(ddd, sprintf("%s/tests_chisq/test.%s.%s.%s.csv", out_fn, cmpnm, spi, chr), quote = FALSE)
